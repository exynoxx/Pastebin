...
suite "pastecache":
  ...
  test "dirty entries are never evicted -> refusal when only dirty bytes remain":
    resetForTest(100)
    check tryAdmit(inlinePaste("a", repeat(30)), repeat(30), "ip1")
    check tryAdmit(inlinePaste("b", repeat(30)), repeat(30), "ip1")
    check tryAdmit(inlinePaste("c", repeat(30)), repeat(30), "ip1")
    # 90 dirty bytes used; a 30-byte paste needs eviction but nothing is clean.
    check not tryAdmit(inlinePaste("d", repeat(30)), repeat(30), "ip1")

  test "clean entries evicted in LRU order":
    resetForTest(100)
    check tryAdmit(inlinePaste("a", repeat(40)), repeat(40), "ip1")
    check markPersisted("a", BlobId(""))
    check tryAdmit(inlinePaste("b", repeat(40)), repeat(40), "ip1")
    check markPersisted("b", BlobId(""))
    # 80 clean bytes; admitting c(40) must evict the LRU clean entry (a).
    check tryAdmit(inlinePaste("c", repeat(40)), repeat(40), "ip1")
    check getDisplayPaste("a").isNone
  ...
  test "markPersisted flips dirty->clean and releases pending":
    ...
    check tryAdmit(inlinePaste("b", repeat(80)), repeat(80), "ip1")
    check getDisplayPaste("a").isNone
